...
- **신속한 피드백 및 개발 속도 향상:** 단위 테스트는 실행 속도가 빠르고 격리되어 있어, 작성한 코드의 결과를 즉각적으로 피드백받을 수 있었습니다. 이는 자연스럽게 **전체적인 개발 속도 향상**에 기여했습니다.

- **Mocking을 통한 의존성 제거:** Repository 계층을 제외한 나머지 Service, Controller 계층의 테스트는 Mocking 기법을 활용해 DB 연결이나 외부 API 호출(Github OAuth)의 무거운 외부 의존성을 제거했습니다.
이를 통해 각 비즈니스 로직(Service)과 요청/응답 처리(Controller)를 격리하여 테스트했고, 테스트 실행 시간을 단축하며 테스트의 안정성 및 효율성을 극대화했습니다.
